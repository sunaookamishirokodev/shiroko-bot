    ...
    for (const file of readdirSync("./src/components/" + dir).filter((f) => f.endsWith(".js"))) {
      ...
      if (dir === "buttons") {
        if (!module.customId || !module.run) {
          log(
            "Unable to load the component " + file + " due to missing 'structure#customId' or/and 'run' properties.",
            "warn"
          );
        ...
        if (!module.customId || !module.run) {
          log(
            "Unable to load the select menu " + file + " due to missing 'structure#customId' or/and 'run' properties.",
            "warn"
          );
        ...
        if (!module.customId || !module.run) {
          log(
            "Unable to load the modal " + file + " due to missing 'structure#customId' or/and 'run' properties.",
            "warn"
          );
